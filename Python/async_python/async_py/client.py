...
class Client:
    ...
    async def get_todos(self) -> list[ToDo]:
        ...
        response = await self.client.get(
            f"https://jsonplaceholder.typicode.com/todos/?_delay=2000&foo={self.rand_int}"
        )
    ...
    async def get_todo(self, todo_id: int) -> ToDo:
        ...
        response = await self.client.get(
            f"https://jsonplaceholder.typicode.com/todos/{todo_id}/?_delay=2000&foo={self.rand_int}"
        )
        response.raise_for_status()
