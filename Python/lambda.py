# Lambda functions
# Anytime that a lambda function could be used, a regular function
...
def square(x: int | float):
    ...
    # and we need to find a place for it, and the logic is defined elsewhere
    return x * x
...
# The filter functions accepts a function and an iterator.
# It returns new iterator with the filter applied now instead of defining a
# separate function -> it's clear we just need meat items from the grocery list
meat_groceries = list(filter(lambda x: x["category"] == "meat", grocery_list))
print(meat_groceries)

# the map function has a similar signature, returns a new iterator but with the lambda fn
# applied to each element
discounted_list = list(map(lambda x: {**x, "price": x["price"] * 0.8}, grocery_list))  # noqa: C417
print(discounted_list)

# We can define a lambda function to inline logic for sorting, which can be useful
# if you need to sort something
